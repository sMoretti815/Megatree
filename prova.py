...
def incorporable(G, Trees):
    ...
    for y in G.nodes():
        ...
        for l in powerlist:
            ...
            for (T, _) in Trees:
                for z in T.nodes():
                    if T.nodes[z]['label'] == G.nodes[y]['label']:
                        if (sorted([T.nodes[node]['label'] for node in T.successors(z)])) == sorted(list(l)):
                            find = True
                            break
            if find == False:
                y_valid = False
